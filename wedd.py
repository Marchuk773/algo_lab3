...
class Wedd():
    ...
    def algorithm(self):
        with open(self.filename, 'r') as file:
            ...
            for line in file:
                ...
                self.add_to_dict(first_number)
                self.add_to_dict(second_number)
                
                first_tribe = self.tribes_marker[self.get(first_number)]
    ...
    def get(self, index):
        return self.numbers_dictionary[index]
    
    def check_gender(self, number, tribe):
        if number % 2 == 1:
    ...
    def add_to_tribe(self, member, newcomer):
        current_tribe = find(self.union_list, self.tribes_marker[self.get(member)])
        self.tribes_marker[self.get(newcomer)] = current_tribe
        self.check_gender(newcomer, current_tribe)
    
    def unite_tribes(self, bigger_number, smaller_number):
        actual_tribe = self.tribes_marker[self.get(bigger_number)]
        removed_tribe = self.tribes_marker[self.get(smaller_number)]
        self.tribes_marker[self.get(smaller_number)] = actual_tribe
